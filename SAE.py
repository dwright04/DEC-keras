...
from DEC import FrameDumpCallback
...
class SAE(object):
    ...
    """
    def __init__(self, dims, act='relu', drop_rate=0.2, batch_size=256, video_path=None):
        self.dims = dims
    ...
    def pretrain_stacks(self, x, epochs=200):
        ...
        for i in range(self.n_stacks):
            ...
            for j in range(3):  # learning rate multiplies 0.1 every 'epochs/3' epochs
                ...
                callbacks = [FrameDumpCallback(features, self.video_path, 'ae_%d'%(i+1))]
    ...
    def pretrain_autoencoders(self, x, epochs=500):
        ...
        for j in range(int(epochs/80)):
            lr = 0.1*pow(10, -j)
            print('learning rate =', lr)
            self.autoencoders.compile(optimizer=SGD(lr, momentum=0.9), loss='mse')
            callbacks = [FrameDumpCallback(x, self.video_path, 'fine-tune')]
